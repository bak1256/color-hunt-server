...
/*
 * 1) READY true/false handler
 */
if (!s.includes("V101069F_READY_HANDLER")) {
  replaceBetween(
    "    paint_ready: (",
...
/*
 * 2) Hunter early-start handler
 */
if (!s.includes("V101069F_EARLY_START")) {
  ...
  const add = `    /* V101069F_EARLY_START */
    early_start_hunt: (
      client: Client,
    ): void => {
      if (this.state.phase !== "paint") {
...
/*
 * 4) Reconnect clock sync + READY resend
 */
{
...
/*
 * 5) Flexible structural verification
 */
const checks = [
  ["V101069F_READY_HANDLER", "READY handler"],
  ["V101069F_EARLY_START", "early-start handler"],
  ["private getPaintReadyState()", "READY state method"],
  ["private sendPaintReadyState", "READY sender method"],
  ["private broadcastPaintReadyState", "READY broadcaster method"],
  ...
  ["allHidersReady:", "allHidersReady payload"],
  ['"paint_ready_state"', "paint_ready_state message"],
];
...
console.log("");
console.log("[done] v0.10.10.69f READY compatibility applied");
